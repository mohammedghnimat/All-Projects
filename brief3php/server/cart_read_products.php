...
//         $query = "SELECT * FROM footsize 
//         JOIN products ON products.product_id = footsize.product_id
//         JOIN cart ON products.product_id = cart.product_id
//         JOIN users ON users.id = cart.user_id
//         WHERE footsize.footsize = ? AND users.id =?";
    
//         $stmt = $pdo->prepare($query);
//         $stmt->execute($data['footsize'],$data['id']);
//         $result = $stmt->fetchAll(PDO::FETCH_ASSOC);
