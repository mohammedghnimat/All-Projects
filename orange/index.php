...
for($i=0;$i<count($phones);$i++){
  echo '<div class="card">';
  foreach($phones[$i] as $a=>$value){
    if($a=='name'){
      echo '<p>'; echo $phones[$i][$a]; echo '</p>';
    }
    if($a=='img_url'){
      printf( '<img src="%s" alt="">',$phones[$i][$a]);
    }
    if($a=='rate'){
       echo "<p>"; printf('rate is %s / 5',$phones[$i][$a]); echo "</p>";
    }
    if($a=='brand'){
      echo '<p>'; echo $phones[$i][$a]; echo '</p>';
    }
    if($a=='price'){
      echo '<h2 style="color:orange;">'; echo $phones[$i][$a];echo'</h2>';
    }
  }

  echo "<button style='Background-color:grey;'>ViewAll<button>";
    echo "</div>";
}
